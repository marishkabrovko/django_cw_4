...
class Recipient(models.Model):
    ...
    full_name = models.CharField(
        max_length=150, verbose_name="ФИО", help_text="Введите ФИО"
    )
...
class Message(models.Model):
    ...
    text = models.TextField(
        verbose_name="Текст сообщения", help_text="Напишите сообщение"
    )
    owner = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
...
class Mailing(models.Model):
    first_sending = models.DateTimeField(
        verbose_name="Начало рассылки", blank=True, null=True
    )
    ...
    recipients = models.ManyToManyField(
        Recipient, verbose_name="Получатели", help_text="Выберите получателей"
    )
    owner = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
...
class AttemptSending(models.Model):
    ...
    status = models.CharField(
        choices=STATUS_OF_ATTEMPT, default="not_success", verbose_name="Статус попытки"
    )
